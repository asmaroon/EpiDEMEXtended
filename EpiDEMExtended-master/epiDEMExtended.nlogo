...
[
  ...
  total-activities-suspended-GP-Policy ;; to keep track of how many activities were not carried out because of freen pass policy hence they had an impact on how the virus spreads
  people-with-0level-gp
  people-with-1level-gp
  people-with-2level-gp
]
...
[
  ...
  green-pass-level    ;; this value maintains the levek of green pass that a person has:
                      ;; more specifically:
                      ;;  0 - no green pass
                      ;;  1- green pass "base"
  ...
  required-level-green-pass-current-target ;; required level of green pass of the current target
  ...
  value current_t_contacts
  value output_list
  value me
  value status
  value my-current-links
]
...
[
  ...
  required-green-pass-level ;; a number that defines the level of green pass needed to access the activity
]
...
to setup-globals
  ...
  set total-activities-suspended-GP-Policy 0
end
...
to setup-leisure-activities
  ...
  [
    ...
    repeat 4 [ set useless file-read ]
  ...
  create-leisure-activities num-activities [
    ...
    set required-green-pass-level file-read
...
to setup-gp-0
  ;; setting up the number of people without a green pass
  file-open "config/GreenPass%.txt"
  let percentage file-read
  ;user-message(word "percentage of people with no green pass " percentage)
  set people-with-0level-gp int ((count people) * percentage / 100)
  ;user-message(word "Total number of people with no green pass " people-with-0level-gp)
  ask n-of people-with-0level-gp people [
  set green-pass-level 0
  ]
end

to setup-gp-1
  ;; setting up the number of people with a green pass base so level 1
  file-open "config/GreenPass%.txt"
  let percentage file-read
  ;user-message(word "percentage of people with green pass base " percentage)
  set people-with-1level-gp int ((count people) * percentage / 100)
  ;user-message(word "Total number of people with green pass base " people-with-1level-gp)
  ask n-of people-with-1level-gp people [
  set green-pass-level 1
  ]
end

to setup-gp-2
  ;; setting up the number of people with a green pass rafforzato so of level 2
  file-open "config/GreenPass%.txt"
  let percentage file-read
  ;user-message(word "percentage of people with green pass rafforzato " percentage)
  set people-with-2level-gp int ((count people) * percentage / 100)
  ;user-message(word "Total number of people with green pass rafforzato " people-with-2level-gp)
  ask n-of people-with-2level-gp people [
  set green-pass-level 2
  ]
end
...
to go
  ...
  ;; class 0 still needs to update their data about contacts and if they are infected
  ask people with [ age-class = 0 ] [ update-your-data ]
...
to update-your-data
  set current_t_contacts [who] of people-here
  set output_list current_t_contacts
  set me [who] of self
  ifelse infected? [
    set status 1
  ]
...
to move  ;; turtle procedure
   ;user-message(word "who am i " [who] of self)
  ;let c count people with [age-class = 0]
  ;user-message(word "num tot 0" c)
  set current_t_contacts [who] of people-here
  set output_list current_t_contacts
  set me [who] of self
  ifelse infected? [
  ...
  set my-current-links my-links
   ;user-message(word "contatti" current_t_contacts)
  ;user-message(word "age class" age-class)
  set output_list fput me output_list
  set output_list remove-duplicates output_list
  set output_list lput status output_list
  ;user-message(word "my data " output_list)
 ; set current_t_contacts [who] of self
 ;if is-list? current_t_contacts[
  ; user-message(word "it is a list")
  ;]
  ;set current_t_contacts [values-from people-here [self]]
  ;user-message(word "contacts " current_t_contacts)
  if age-class = 1 [
...
to handle-cycle [ activities-list durations-list ]
  ...
  ifelse not moving? [
    ifelse current-activity-time-left = 0 [
      ...
      ;; applying the green pass policy so checking if the person has the green pass level required by the activity
      ;let temp item current-activity-index activities-list

      if any? (leisure-activities-on target)[
        set required-level-green-pass-current-target [required-green-pass-level] of leisure-activities-on target
      ]
      if any? (health-activities-on target)[
         set required-level-green-pass-current-target [required-green-pass-level] of health-activities-on target
      ]
      if any? (education-activities-on target)[
         set required-level-green-pass-current-target [required-green-pass-level] of education-activities-on target
      ]
      if any? (professional-activities-on target)[
         set required-level-green-pass-current-target [required-green-pass-level] of professional-activities-on target
      ]
      ;user-message(word "gpl " required-level-green-pass-current-target)
      if not (current-activity = "Home")[
        let required-gp-level 0
        ifelse is-list?  required-level-green-pass-current-target[
          set required-gp-level first  required-level-green-pass-current-target
        ]
        [
          set required-gp-level  required-level-green-pass-current-target
        ]
        if (green-pass-level < required-gp-level)[
          ;this means that the level of green pass I have is not enough to do that specific activity, hence I cannot proceed with this type of target
          set target home-patch
          set total-activities-suspended-GP-Policy total-activities-suspended-GP-Policy + 1
        ]
      ...
      ;or any? (education-activities-on target) or any? (health-activities-on target) or any? (professional-activities-on target)
...
333
747
Activities suspended because of Green Pass policy
total-activities-suspended-GP-Policy
17
1
11
...
332
634
People with reinforced green pass
people-with-2level-gp
17
1
11
